...
| 世界手 | 点/拖 slop 分离；`state.__hand` 握持；拖着的固体也参与碰撞；扫掠顶住不穿模；空地套索 / Shift 多选；键先打手里的单位。不是刚体引擎，套索不画橡皮筋 |
...
9. 世界手能握、能顶、能多选，仍不是 Unity 级物理 / 输入栈

## 下一刀（质量，不再铺接口）

1. 世界手的手感（套索可视、编组不散、墙滑更稳），不要再修图核 inset
2. 栏宽 compose 的视觉密度（标题/图/文的投稿级间距）
3. 剪辑轨仍必须是插件属性；不要加关键字
4. 不要宣称 Nature 级或已超过 Claude Science
