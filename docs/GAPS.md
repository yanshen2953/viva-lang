...
7. 小栏宽 mm 图默认跟手 `__tip`（空字不画，打印无鬼影），不再靠常驻角 HUD。不写 `areaX` 时编译器按场景估绘图区，并停在作者节点/手写 frame 腾出的最大空矩形；两张以上未绑 chart 自动成网格时同样避开题注，满幅氛围层不占空位。inset 按 chrome 溢出整量增长，直到绘图区下限（约 22%）；触底后仍可能省略或收回格内，不是投稿成品 typesetter
