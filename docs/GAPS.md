...
评估：2026-08-24。分支 `cursor/mcp-pi-exam-f1b5`。接续说明：`docs/HANDOFF.md`。
...
| 3 通用 Agent 接口 | 齐 | **接入齐 / 闭环加严**：CLI/MCP/HTTP 能编能导；确定性 repair 进 session；visual 错误失败 compile success；离线 exam 种子编译率在 CI。LLM hard exam **8/8**（`deepseek-v4-flash-vision-exp`，Pi 经 MCP `viva mcp` stdio；H05 现有 `data` 表） |
...
5. session 会修 overflow / 空栏绑 data / 补轴题；visual 错误失败 success。LLM hard exam **8/8**（MCP 接 Pi；多数 `pi -p` 会顶满 240s，runner 从 stdout 捞 `artifact`）
