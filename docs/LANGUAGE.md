...
- `chart.heatmap`：`valueField` + `zlim`，右侧连续色条。色条宽高按场景比例（mm 不再把 10/40 px 当成毫米），`zLabel` 在色标数字右侧 −90° 竖排，和 `yLabel` 同一套轴标题，按绘图区高度折行。色条刻度落在 `zlim` 值上（整数 0…4 每隔一档；更宽的域走 nice 并键两端），不再只标底/中/顶三段；顺序色按索引插值，色带是细条渐变。未写 `cellW`/`cellH` 时按相邻唯一 x/y 的中位步长铺格；离散数值轴刻度落在格心（`xlim: -0.5 7.5` 不再把 −0.5 / 7.5 画成刻度）；格子白缝按短边比例，不是 1 个场景单位。热图 Y 不翻转：第一行 / 最小 row 在顶上（和数据表一样），不是笛卡尔底边
