...
| 排版 | 作者只说「2×2 图、单栏 89 mm、安全框」；格子、出血、字幕条、对位由编译器算 | `layout.figure` 网格（不写 `inset*` 时按绑定 chart 估留白）；`layout.board` 安全框 + `splits` / `beats` / `bleed` / `typeGrid`；`unit: mm` + 单/双栏；`--beats` 出 PNG 序列 | 无跨页；估 inset ≠ 碰撞求解；`play` 仍是拍遮罩，不是成片 |
...
4. `layout.board play` / `typeGrid` 已在；`--beats` 只是 PNG 序列，不是成片时间轴 / 视频文件
