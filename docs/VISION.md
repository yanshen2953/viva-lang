...
没有：完整 CJK 字库、通用排版求解（跨页、栏宽文法）。有 time / box / violin（KDE 轮廓）/ 显著性括号；轴刻度已离开数据域。chrome 盒子会互推一档（标题避开 `(a)`，y 轴标题避开刻度，图例避开色条）；图/轴标题按栏宽折行（最多三行），重叠刻度抽稀，相邻格 chrome 互叠时再长 inset。仍不是 InDesign。小栏宽 mm 图默认不再画常驻 HUD 读数。
...
1. 跨页 / 栏宽文法（图/轴标题已按栏宽折行，重叠刻度会抽稀，相邻格会再让 inset，还不是通用排版器）
