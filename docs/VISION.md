...
没有：完整游戏级过渡曲线。内联卡有只读结构检查条，仍无 visual/raster、也没有自动修复。`__sel.keys` 默认让其它面板 **藏行**（含 box / violin / 折线；box 四分位、violin KDE 和折线线段按选中行重算/重连；`link: dim` 可改回变淡）。Runtime 用 CSS `opacity` + 命中组 `scale` 做 220ms 缓动（含 `layout.board play` 拍遮罩）；box / 折线摘要几何也按同一时段插值；violin 路径 `d` 与静态导出仍硬切。不是时间轴。
...
2. `__sel` 已是共享 key 集并藏行；box / violin / 折线按选中行重算或重连；高亮、play 遮罩和 box/折线几何走 220ms 缓动，仍缺时间轴
