...
没有：完整 CJK 字库、通用排版求解（换行/减刻度/跨格避让）。有 time / box / violin（KDE 轮廓）/ 显著性括号；轴刻度已离开数据域。chrome 盒子会互推一档（标题避开 `(a)`，y 轴标题避开刻度，图例避开色条），仍不是 InDesign。小栏宽 mm 图默认不再画常驻 HUD 读数。
...
1. 轴标题换行 / 跨格避让（图标题已按栏宽折行，重叠刻度会抽稀，还不是通用排版器）
