...
| 排版 | 作者只说「2×2 图、单栏 89 mm、安全框」；格子、出血、字幕条、对位由编译器算 | `layout.figure` 网格（不写 `inset*` 时按绑定 chart 估留白）；`layout.board` 安全框 + `splits` / `beats` / `bleed` / `typeGrid`；`unit: mm` + 单/双栏；`--beats` 出 PNG 序列，可选 ffmpeg 拼 GIF/MP4 幻灯 | 无跨页；估 inset ≠ 碰撞求解；`play` 仍是拍遮罩，不是成片时间轴 |
...
没有：完整游戏级过渡曲线、内联卡上的检查/修复壳。`__sel.keys` 默认让其它面板 **藏行**（含 box / violin / 折线线段；`link: dim` 可改回变淡）。Runtime 用 opacity + 命中组 `scale` 做 220ms 缓动，静态导出仍是硬切。不是时间轴。
...
没有：完整 CJK 字库、通用排版求解（跨页、栏宽文法）。有 time / box / violin（KDE 轮廓）/ 显著性括号；轴刻度已离开数据域。chrome 盒子会互推一档（标题避开 `(a)`，y 轴标题避开刻度，图例避开色条）；图/轴标题按栏宽折行（最多三行），图例键按剩余栏宽折行（含连字符，最多两行），重叠刻度抽稀，相邻格 chrome 互叠时再长 inset。仍不是 InDesign。小栏宽 mm 图默认不再画常驻 HUD 读数。

### 3.3 图像 / 视频级排版

有：`layout.figure` 网格 + `(a)(b)` + 格子甲板；不写 `inset*` 时编译器按该格 chart 的刻度/标题/图例/色条迭代估留白；不写 `x/y/w/h` 时铺满场景，或 `panel: body` 吃 board 槽；`title`/`subtitle`/`caption` 由编译器画；两张以上未绑 panel 的 chart 自动成网格；`layout.board` 的 `safe` / `title` / `body` / `lower` + 题注属性 + `splits` / `beats` / `bleed` / `typeGrid`；`unit: mm` + 栏宽；CLI/MCP/HTTP `--beats` 按 `__beat` 导出 PNG 序列，`-f gif|mp4` 用 ffmpeg 把这些栅格拼成幻灯（2 fps，不是时间轴 / 成片）。  
...
1. 跨页 / 栏宽文法（图/轴标题和图例键已按栏宽折行，重叠刻度会抽稀，相邻格会再让 inset，还不是通用排版器）
2. `__sel` 已是共享 key 集并藏行；高亮有 scale 缓动，仍缺时间轴和按行摘要图种过滤
3. 再扩 CJK 或允许宿主挂全库；缺字仍可能 `?`
4. `layout.board play` / `typeGrid` 已在；`--beats` 默认 PNG 序列，`-f gif|mp4` 只是 ffmpeg 幻灯，不是成片时间轴
