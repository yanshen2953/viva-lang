...
| Beat playback | `npx vite-node src/cli.ts -- export examples/storyboard.viva --beats -f gif -o /tmp/story.gif` (samples the same Clock hold+ease cycle at timeline fps, then ffmpeg encodes it; CI installs ffmpeg, tests skip without it) |
