...
- Charts emit __hover / __brush (dx* data-domain, frame + xField for linked panels; a real brush stays on after dragend, empty click clears; a long trail becomes a lasso via inside()) / __highlightGrp / __sel.keys. Other panels hide rows and box/violin/line summaries outside __sel; box quartiles recompute from selected rows (link: dim to fade). Matched group marks scale up; Runtime eases CSS opacity/transform (~220ms, including play veils), not a timeline. interactive: false disables them.
