...
- Multi-panel: widget layout.figure (cols/rows, optional title/subtitle/caption) then widget chart.* with panel: a (not areaX / inset* / panel-deck / page-title magic numbers; compiler estimates chrome, wraps long chart/axis titles and legend keys, ellipsizes a capped last line, keeps neighbor chrome apart, paints cell decks + (a) labels + title copy). Chart span: 2 (or colspan) occupies two columns. Omit figure x/y/w/h to fill the scene, or panel: body to fill a board slot. An author node with role: panel or role: plot and x/y/w/h becomes a frame (xlim/ylim on the node copy over); figure/chart bind with panel: name, World marks with frame: name. Omit gutter/margin/titleH — compiler sizes them in scene units (mm gutters and title bands stay in millimetres). Single-column mm charts can omit areaX/areaY; the compiler fills leftover scene space after author nodes/frames. Two or more charts with no panel/area become a figure grid in leftover scene space (author captions stay visible; atmosphere/full-bleed is ignored).
- Video/image board: widget layout.board (safe/title/body/lower; title/subtitle/caption props; controls + bind paint HUD chips (selected chip is opaque, no extra value glyph) and a hud slot; omit w/h to fill the scene; omit safe/titleH/lowerH — compiler sizes those bands from copy and chips; splits: 2 → left/right; beats: 4 → beat0..; bleed: 16 → bleed/trim + crop marks; typeGrid: true → baseline + type0.. columns; play: true advances __beat). Export --beats writes one PNG per beat; --beats -f gif|mp4 is an ffmpeg slideshow, not a timeline. Scene unit: mm and column: single (89) or double (183). page: a4|letter sets omitted page size; a taller mm scene slices PDF into that page height and stamps n / N (continued figure title on later slices). Figure rows that would straddle the page knife move to the next page and the scene grows; SVG/PNG stay one canvas, not a column reflow. PDF CJK uses a bundled subset unless the host sets VIVA_PDF_CJK_FONT / --cjk-font / cjkFontPath. Frame xScale/yScale: log|band|time. ISO dates auto-time. String columns auto-band. Legend defaults outside (legend: right|bottom|inside|false).
- Charts emit __hover / __brush (dx* data-domain, frame + xField for linked panels; a real brush stays on after dragend, empty click clears; a long trail becomes a lasso via inside()) / __highlightGrp / __sel.keys. Other panels hide rows and box/violin/line summaries outside __sel; box quartiles, violin KDE, and line segments recompute from selected rows (link: dim to fade). Matched group marks scale up; Runtime eases CSS opacity/transform, box/line summary geom, and matching violin path d (~220ms, including play veils; unlike skeletons still snap), not a timeline. interactive: false disables them.
