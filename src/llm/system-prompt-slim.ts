...
- Video/image board: widget layout.board (safe/title/body/lower; title/subtitle/caption props; controls + bind paint HUD chips and a hud slot; omit w/h to fill the scene; omit safe/titleH/lowerH — compiler sizes those bands from copy and chips; splits: 2 → left/right; beats: 4 → beat0..; bleed: 16 → bleed/trim + crop marks; typeGrid: true → baseline + type0.. columns; play: true advances __beat). Export --beats writes one PNG per beat; --beats -f gif|mp4 is an ffmpeg slideshow, not a timeline. Scene unit: mm and column: single (89) or double (183). Frame xScale/yScale: log|band|time. ISO dates auto-time. String columns auto-band. Legend defaults outside (legend: right|bottom|inside|false).
- Charts emit __hover / __brush (dx* data-domain, frame + xField for linked panels; a real brush stays on after dragend, empty click clears; a long trail becomes a lasso via inside()) / __highlightGrp / __sel.keys. Other panels hide rows and box/violin/line summaries outside __sel (link: dim to fade). Matched group marks scale up; Runtime eases CSS opacity/transform (~220ms, including play veils), not a timeline. interactive: false disables them.
