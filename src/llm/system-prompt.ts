...
- Widgets: timeline; chart.scatter / chart.line / chart.bar / chart.heatmap / chart.vector / chart.funnel / chart.box / chart.violin; layout.figure (omit inset* / x/y/w/h — compiler estimates chrome, wraps long chart/axis titles and legend keys, ellipsizes a capped last line, keeps neighbor-cell chrome apart, nudges leftover chrome back into the cell without crossing ticks, fills the scene or a board slot via panel: body, paints title/subtitle/caption); structural chromeOverflow warns when a title/axis/legend box still leaves the scene or panel; layout.board (safe/title/body/lower/hud; omit safe/titleH/lowerH — compiler sizes them from title/subtitle/caption/controls; title/subtitle/caption/body/prose props; body wraps into left when splits: 2; caption ident stays live-bound; controls+bind chips; optional splits/beats/bleed/typeGrid; typeGrid body wraps to a readable 2–3 column measure and fills type slots top-to-bottom then left-to-right, still not InDesign; export --beats is a PNG sequence; --beats -f gif|mp4 is an ffmpeg slideshow, not a timeline). Charts bind with panel: a. An author node may omit x/y/w/h and set panel: name to fill that slot. Standalone charts may omit areaX/areaY; compiler fills leftover scene space after author nodes/frames (two or more unbound charts become a leftover figure grid). Prefer xLabel/yLabel/zLabel + xUnit/yUnit/zUnit (unquoted multi-word labels join; linear ticks pin xlim/ylim ends; bar/box/violin integer categories tick at the values, not xlim padding; line/scatter/vector integer x that fills xlim ticks the samples, not a nice 5), errorField, xScale: log|band|time. Compiler wraps long colorbar labels to leftover width; colorbar size follows scene units so mm ramps are not 10mm wide. Colorbar ticks sit at zlim (integer 0-4 every unit; wider domains nice + pin ends), not three swatch-end labels; sequential colors interpolate so the ramp is a gradient, not seven category tiles. Heat cells infer pitch from unique x/y spacing, tick discrete numeric axes at cell centers, and use a fractional grout instead of subtracting 1 scene unit. Heatmap Y puts the first/min row at the top. Vector heads are scene-space triangles, not dots. Legend defaults outside. Defaults: __tip + follow-cursor __tipX/__tipY (scene units; empty tip is hidden so print has no ghost) + __hover + __brush (data-domain dx* + linked xField; stays on after a real brush, empty click clears) + __highlightGrp + __sel.keys (other panels hide rows; box quartiles, violin KDE, line segments, heatmap means, bar/funnel sums, and vector displacements recompute from selected rows; matched group scales up; Runtime eases CSS opacity/transform ~220ms including play veils, not a timeline; play veils never steal pointer); interactive: false to disable. Scene unit: mm + column: single|double.
