...
- Space: declare frame blocks with x/y scene ranges and xlim/ylim data domains; set node prop frame: <name> so x/y are data-domain values. Framed World marks inherit chart __tip / __hover / __highlightGrp / __sel (brush when x/y are data fields).
- Widgets: timeline; chart.scatter / chart.line / chart.bar / chart.heatmap / chart.vector / chart.funnel / chart.box / chart.violin; layout.figure (omit inset* / x/y/w/h — compiler estimates chrome, wraps long chart/axis titles and legend keys, ellipsizes a capped last line, keeps neighbor-cell chrome apart, fills the scene or a board slot via panel: body, paints title/subtitle/caption); layout.board (safe/title/body/lower/hud; omit safe/titleH/lowerH — compiler sizes them from title/subtitle/caption/controls; title/subtitle/caption props; controls+bind chips; optional splits/beats/bleed/typeGrid; export --beats is a PNG sequence; --beats -f gif|mp4 is an ffmpeg slideshow, not a timeline). Charts bind with panel: a. Standalone charts may omit areaX/areaY; compiler fills leftover scene space after author nodes/frames (two or more unbound charts become a leftover figure grid). Prefer xLabel/yLabel/zLabel + xUnit/yUnit/zUnit, errorField, xScale: log|band|time. Compiler wraps long colorbar labels to leftover width. Legend defaults outside. Defaults: __tip + __hover + __brush (data-domain dx* + linked xField; stays on after a real brush, empty click clears) + __highlightGrp + __sel.keys (other panels hide rows; box quartiles, violin KDE, and line segments recompute from selected rows; matched group scales up; Runtime eases CSS opacity/transform ~220ms including play veils, not a timeline); interactive: false to disable. Scene unit: mm + column: single|double.
